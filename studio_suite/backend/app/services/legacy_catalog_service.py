...
def get_legacy_catalog(db: Session) -> dict:
    price_by_service_id = _latest_prices_by_service_id(db, salon_id=1)

    services = db.query(ServiceCatalogItem).order_by(ServiceCatalogItem.legacy_code.asc()).all()
    service_prices = []
    ...
    bundles = db.query(BundleCatalog).order_by(BundleCatalog.legacy_code.asc()).all()
...
def update_service_price(db: Session, service_id: int, salon_id: int, price: float) -> dict:
    ...
    affected_bundle_ids = [
        ...
        .filter(BundleCatalogItem.service_id == service_id)
        .all()
    ]
...
def update_bundle_item_price(db: Session, bundle_id: int, position: int, override_price: float | None) -> dict:
    ...
    _recalculate_bundle_price(db, bundle_id=bundle_id, salon_id=1)
    db.commit()
    db.refresh(item)
    return {
        "bundle_id": bundle_id,
...
def create_bundle(db: Session, bundle_code: str, bundle_name: str, salon_id: int = 1) -> dict:
    existing = db.query(BundleCatalog).filter(BundleCatalog.legacy_code == bundle_code).first()
    if existing:
