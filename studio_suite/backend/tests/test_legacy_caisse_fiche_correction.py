...
from app.schemas.legacy_caisse import LegacyCaisseCashSessionWrite, LegacyCaisseFicheCreate, LegacyCaisseLineWrite, LegacyCaissePaymentAllocationWrite
...
def test_void_fiche_marks_sale_and_payment_void_and_excludes_daily_summary(db_session):
    ...
    result = asyncio.run(void_fiche(created.sale_id, user, db_session))
...
def test_duplicate_void_is_idempotent(db_session):
    ...
    first = asyncio.run(void_fiche(created.sale_id, user, db_session))
    second = asyncio.run(void_fiche(created.sale_id, user, db_session))
...
def test_void_fiche_is_blocked_after_cash_session_is_closed(db_session):
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(void_fiche(created.sale_id, user, db_session))
